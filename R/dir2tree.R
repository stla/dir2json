.dir2tree <- function(dir, depth, vertical){
  .C(
    "dirToTree", depth = as.integer(depth), dir = as.character(dir),
     vertical = as.integer(vertical), result = character(1L), NACK = TRUE
  )[["result"]]
}

#' @title Tree representation of a directory
#' @description Returns the hierarchical structure of a folder as a tree.
